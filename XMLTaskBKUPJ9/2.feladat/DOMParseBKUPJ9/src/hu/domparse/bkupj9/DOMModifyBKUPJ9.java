...
public static void main(String[] args) throws ParserConfigurationException, IOException, SAXException, XPathExpressionException, TransformerException { {
		...
		for (int i = 0; i < masodikMonitorozásNodeList.getLength(); i++) { //A 2. monitorozás elem aktuális idájárásának megváltoztatása naposra
			Node element = masodikMonitorozásNodeList.item(i);
			if ("aktuálisIdájárás".equals(element.getNodeName())) {
				element.setTextContent("napos");
			}
		}
		
		Element monitorozo = document.createElement("monitorozó"); //A 2. monitorozás elemhez hozzáadjuk a monitorozó nevût
